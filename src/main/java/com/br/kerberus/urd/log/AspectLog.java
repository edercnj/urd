...
public class AspectLog {
	...
	public void logUrdException(JoinPoint joinPoint, UrdException e) {
		log.info(String.format("Launch Managed Exception: {{%s}}  - method: {{%s}} - line {{%s}}", e.getClass().getName(), joinPoint.getSignature(), e.getStackTrace()[0].getLineNumber()));
	}
	...
	public void logMethodReturn(JoinPoint joinPoint, Object result) {
		...
		if (result != null)
			...
			log.info(String.format("Method return: {{%s}} -return {{ [null or void] }}", joinPoint.getSignature()));
	}
